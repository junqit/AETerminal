...
public class AEAIResponseParser {
    ...
    public func parseResponse(_ response: AnyObject) -> [ParsedResponse] {
        AELog("🔍 AEAIResponseParser - 开始解析响应: \(response)")

        var results: [ParsedResponse] = []

        // 解析响应数据
        if let dict = response as? [String: Any] {
            AELog("📦 响应是字典类型，键: \(dict.keys.sorted())")
            ...
            if let llmResponsesArray = dict["llm_responses"] as? [[String: Any]] {
                ...
                AELog("✅ 找到 llm_responses 数组，共 \(llmResponsesArray.count) 项")
                for (index, responseDict) in llmResponsesArray.enumerated() {
                    AELog("📝 解析第 \(index + 1) 个响应: \(responseDict.keys.sorted())")
                ...
                AELog("✅ 找到 llm_responses 字典，键: \(llmResponsesDict.keys.sorted())")
                ...
                for key in sortedKeys {
                    if let responseDict = llmResponsesDict[key] as? [String: Any] {
                        AELog("📝 解析 \(key) 响应: \(responseDict.keys.sorted())")
            ...
            // 其他类型，直接转换为字符串描述
            AELog("⚠️ 响应类型未知: \(type(of: response))")
            let text = "\(response)"
            results.append(ParsedResponse(text: text, isMarkdown: false))
        }

        AELog("✅ AEAIResponseParser - 解析完成，共 \(results.count) 个响应")
        for (index, result) in results.enumerated() {
            AELog("   响应 \(index + 1): \(result.assistantName)")
            AELog("   内容长度: \(result.text.count)")
            AELog("   是否 Markdown: \(result.isMarkdown)")
            AELog("   内容预览: \(result.text.prefix(100))...")
        }
        return results
    }
    ...
    private func debugPrintDictStructure(_ dict: [String: Any], indent: Int = 0) {
        ...
        for (key, value) in dict.sorted(by: { $0.key < $1.key }) {
            if let dictValue = value as? [String: Any] {
                AELog("\(indentStr)\(key): [Dictionary]")
                debugPrintDictStructure(dictValue, indent: indent + 1)
            } else if let arrayValue = value as? [[String: Any]] {
                AELog("\(indentStr)\(key): [Array of Dictionary] (count: \(arrayValue.count))")
                if let first = arrayValue.first {
                    debugPrintDictStructure(first, indent: indent + 1)
                }
            } else if let arrayValue = value as? [String] {
                AELog("\(indentStr)\(key): [Array of String] (count: \(arrayValue.count))")
                if let first = arrayValue.first {
                    AELog("\(indentStr)  [0]: \(first.prefix(50))...")
                }
            } else {
                let valueStr = "\(value)"
                let preview = valueStr.prefix(100)
                AELog("\(indentStr)\(key): \(type(of: value)) = \(preview)...")
            }
        }
    }
}
