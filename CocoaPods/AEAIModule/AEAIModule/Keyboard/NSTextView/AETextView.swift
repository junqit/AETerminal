...
public class AETextView: NSView, NSTextViewDelegate {
    ...
    public func textDidChange(_ notification: Notification) {
        ...
        // 实时回调当前输入内容
        let textPackage = AETextPackage(text: text)
        delegate?.aeTextView(self, didChangeInput: textPackage)
    ...
    public func textView(_ textView: NSTextView, doCommandBy commandSelector: Selector) -> Bool {
        ...
        if commandSelector == #selector(NSResponder.insertNewline(_:)) {
            ...
            // 创建文本包
            let textPackage = AETextPackage(text: inputText)

            // 通知代理输入内容
            delegate?.aeTextView(self, didInputText: textPackage)
...
extension AETextView: AECombinationKeyHandler {
    ...
    public func handleCombinationKey(event: NSEvent, modifiers: NSEvent.ModifierFlags, key: String) -> Bool {
        ...
        if modifiers.contains(.command) && key.lowercased() == "i" {
            AELog("⌨️ Command+I 激活 AETextView")
            focus()
            return true
        }
