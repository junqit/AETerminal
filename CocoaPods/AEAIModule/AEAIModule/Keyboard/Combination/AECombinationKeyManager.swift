...
public class AECombinationKeyManager {
    ...
    public func register(_ handler: AECombinationKeyHandler) {
        ...
        if handlers.contains(where: { $0.handler?.combinationKeyHandlerID == handler.combinationKeyHandlerID }) {
            if enableDebugLog {
                AELog("⚠️ Handler '\(handler.combinationKeyHandlerID)' 已注册")
            }
            return
        }
        ...
        if enableDebugLog {
            AELog("✅ 注册 Handler: \(handler.combinationKeyHandlerID)")
        }
    ...
    public func unregister(_ handler: AECombinationKeyHandler) {
        ...
        if enableDebugLog && handlers.count < countBefore {
            AELog("🗑️ 注销 Handler: \(handler.combinationKeyHandlerID)")
        }
    ...
    private func handleKeyEvent(_ event: NSEvent) -> Bool {
        ...
        if isFunctionKey {
            if enableDebugLog {
                AELog("⌨️ 功能键事件: \(key) (keyCode: \(keyCode))")
            }
        }
        // 非功能键必须有修饰键才处理（避免拦截普通字符输入）
        else if !cleanModifiers.isEmpty {
            if enableDebugLog {
                AELog("🔑 组合键事件: \(formatModifiers(cleanModifiers)) + \(key)")
            }
        }
        // 既不是功能键也没有修饰键，不处理
        else {
        ...
        for handler in currentHandlers {
            ...
            if handled {
                if enableDebugLog {
                    AELog("✅ Handler '\(handler.combinationKeyHandlerID)' 消费了事件")
                }
