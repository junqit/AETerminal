...
/// 目录选择回调协议
public protocol AELeftViewDelegate: AnyObject {
    /// 当用户确认选择目录时调用
...
/// 左侧视图焦点委托协议
public protocol AELeftViewFocusDelegate: AnyObject {
    /// 当 leftView 获得焦点时调用
...
class DirectoryTableCellView: NSTableCellView {
    ...
    func configure(with item: DirectoryItem, isSelected: Bool) {
        ...
        if isSelected {
            ...
            AELog("✅ 设置选中: \(item.name), 背景色: \(selectedColor), hidden: \(backgroundView.isHidden)")
        } else {
            // 未选中状态：隐藏背景 + 默认文字颜色
...
/// 左侧目录列表视图
public class AELeftView: NSView {

    // MARK: - Properties

    public weak var delegate: AELeftViewDelegate?
    ...
    public func loadDirectories(atPath path: String) {
        rootPath = path
        currentPath = path
        selectedItem = nil // 清除选中项

        // TODO: 移除 AEAIEnginModule 依赖后需要重新实现
        // 加载根目录的子目录
...
//        }
    ...
    /// 激活并选中第一个目录
    public func focusAndSelectFirst() {
        // 成为第一响应者
    ...
    private func toggleDirectory(at index: Int) {
        ...
        if item.isExpanded {
            ...
            AELog("展开目录: \(item.name), 子目录数: \(item.children?.count ?? 0)")
        }
    ...
    private func selectDirectory(at index: Int) {
        ...
        AELog("选中目录: \(item.name) - \(item.fullPath)")
...
extension AELeftView: NSTableViewDelegate {
    ...
    public func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        if isSelected {
            AELog("🔵 Row \(row) (\(item.name)) 应该显示为选中状态")
        }
...
extension AELeftView: AECombinationKeyHandler {
    ...
    public func handleCombinationKey(event: NSEvent, modifiers: NSEvent.ModifierFlags, key: String) -> Bool {
        // 处理 Command+L：激活左侧视图
        if modifiers.contains(.command) && key.lowercased() == "l" {
            AELog("⌨️ Command+L 激活 AELeftView")
            focusAndSelectFirst()
            return true
        }
