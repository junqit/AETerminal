...
@objc public final class AEModuleCenter: NSObject {
    ...
    // MARK: - Account Forwarding (Public Class Methods)

    /// 用户登录成功
    @objc public class func userDidLogin() {
        let snapshot = shared.getModulesSnapshot()
        for module in snapshot {
            module.userDidLogin?()
        }
    }
    ...
    /// 用户退出登录
    @objc public class func userDidLogout() {
        let snapshot = shared.getModulesSnapshot()
        for module in snapshot {
    ...
    /// 用户信息发生变更
    @objc public class func userInfoDidUpdate() {
        let snapshot = shared.getModulesSnapshot()
        for module in snapshot {
            module.userInfoDidUpdate?()
        }
    }

    // MARK: - Privacy Forwarding (Public Class Methods)

    /// 用户同意隐私协议
    @objc public class func privacyDidAgree() {
        let snapshot = shared.getModulesSnapshot()
        for module in snapshot {
            module.privacyDidAgree?()
        }
    }

    /// 用户拒绝隐私协议
    @objc public class func privacyDidDecline() {
        let snapshot = shared.getModulesSnapshot()
        for module in snapshot {
            module.privacyDidDecline?()
        }
    }

    /// 隐私协议弹框即将展示
    @objc public class func privacyWillShow() {
        let snapshot = shared.getModulesSnapshot()
        for module in snapshot {
            module.privacyWillShow?()
        }
    }

    /// 隐私协议弹框已关闭
    @objc public class func privacyDidDismiss() {
        let snapshot = shared.getModulesSnapshot()
        for module in snapshot {
            module.privacyDidDismiss?()
        }
    }
