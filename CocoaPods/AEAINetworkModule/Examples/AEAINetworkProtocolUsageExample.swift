...
class AINetworkService {
    ...
    func sendCustomMessage(messageType: String, content: [String: Any]) {
        ...
        do {
            ...
            AELog("✅ 自定义消息已发送: \(messageType)")
        } catch {
            AELog("❌ 自定义消息发送失败: \(error)")
        }
    }
        ...
        if networkModule.isConnected {
            ...
            networkModule.connect { success, error in
                if success {
                    ...
                    AELog("❌ 网络连接失败: \(error?.localizedDescription ?? "")")
                }
                completion(success)
            }
        }
...
class BatchRequestHandler {
    ...
    func sendBatchRequests(_ requests: [AENetReq]) {
        AELog("📦 开始批量发送 \(requests.count) 个请求...")

        for (index, request) in requests.enumerated() {
            networkModule.sendAsync(request) { result in
                switch result {
                case .success:
                    AELog("✅ 请求 #\(index + 1) 发送成功")
                case .failure(let error):
                    AELog("❌ 请求 #\(index + 1) 发送失败: \(error)")
                }
            }
        }
    }
