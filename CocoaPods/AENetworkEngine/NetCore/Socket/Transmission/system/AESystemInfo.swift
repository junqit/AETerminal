...
public class AESystemInfoManager {
    ...
    public func sendSystemInfo(via io: AEIOProtocol, completion: ((Bool) -> Void)? = nil) {
        ...
        io.send(data: data) { success, error in
            if success {
                AELog("[AESystemInfo] ✓ 已发送本地系统信息")
            } else {
                AELog("[AESystemInfo] ✗ 发送系统信息失败: \(error?.localizedDescription ?? "unknown")")
            }
            completion?(success)
        }
    }
    ...
    public func receiveSystemInfo(_ data: Data) -> Bool {
        ...
        AELog("[AESystemInfo] ✓ 收到对端系统信息:")
        AELog("  - 设备类型: \(info.deviceType)")
        AELog("  - 系统: \(info.systemName) \(info.systemVersion)")
        AELog("  - 应用版本: \(info.appVersion)")
