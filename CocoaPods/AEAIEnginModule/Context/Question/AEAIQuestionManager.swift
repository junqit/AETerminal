...
internal class AEAIQuestionManager {
    ...
    private func loadQuestions() {
        ...
        do {
            ...
            AELog("Failed to load question history for context \(contextID): \(error)")
            questions = []
        }
    }
    ...
    private func saveQuestions() {
        do {
            let dictArray = questions.map { $0.toInfoMap() }
            let data = try JSONSerialization.data(withJSONObject: dictArray, options: .prettyPrinted)
            try data.write(to: historyFileURL, options: .atomic)
        } catch {
            AELog("Failed to save question history for context \(contextID): \(error)")
        }
    }
}
